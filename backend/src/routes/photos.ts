...
router.post(
  ...
  async (req: MulterRequest, res: Response, next: NextFunction) => {
    try {
      ...
      // Optimize image to all sizes (async, non-blocking)
      logger.info({
      ...
      // Process image asynchronously using queue
      const processingResult = await processImageAsync(req.file.buffer, {
        userId,
        priority: 'high', // High priority for user uploads
        metadata: {
      ...
      // Get optimized images from result
      if (!processingResult.allSizes) {
        throw new Error('Image processing failed - no sizes generated');
      }

      const optimized = processingResult.allSizes;
